...
//fontawesome
import { faCog, faChevronRight, faMoneyBillWave, faPlusCircle, faPencilAlt, faTrashAlt, 
  faHomeAlt, faSearch, faFileExcel, faFileZipper, faCalendarDays, faTag} from '@fortawesome/free-solid-svg-icons'; 
import { faSlideshare} from '@fortawesome/free-brands-svg-icons'; 
import { AngularFaviconService } from 'angular-favicon';

@Component({
  selector: 'app-aplicant-list',
  templateUrl: './aplicant-list.component.html',
  styleUrls: ['./aplicant-list.component.sass']
})
export class AplicantListComponent {
  ...
  setValue() {
    ...
    this.backendService.betweenDate_JobApplication(this.start, this.end).subscribe(data => {
  ...
  deleteJobApplication(id: any): void{
    this.backendService.delete_JobApplication(id).subscribe({
      next: (res) => {
        this.submitted  = true;
        let message = `Successfully delete Job Application Data with ID: ${id}`;
        this.showToasterSuccess(message);
      }
    })
  }
  ...
  confirmBox(id: any){
    Swal.fire({
      ...
      if (result.value) {
        this.deleteJobApplication(id);
        
        Swal.fire(
          'Deleted!',
          'Job Application Data has been deleted.',
          'success'
        )
      } else if (result.dismiss === Swal.DismissReason.cancel) {
        Swal.fire(
          'Cancelled',
          'Job Application Data is Safe',
          'error'
        )
      }
    })
